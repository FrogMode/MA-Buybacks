...
var _chunkDDBRY6AFjs = require('./chunk-DDBRY6AF.js');
...
var generateExternal = async (external) => {
  ...
  for (const item of external) {
    ...
    const deps = await _chunkXVMTOYRUjs.getProductionDeps.call(void 0, pkgPath);
    result.push(...deps);
  }
  return result;
};
async function runEsbuild(options, {
  ...
  if (result && result.warnings && !_chunkXVMTOYRUjs.getSilent.call(void 0, )) {
...
// src/api-extractor.ts
var logger2 = _chunkXVMTOYRUjs.createLogger.call(void 0, );
...
async function build(_options) {
  const config = _options.config === false ? {} : await _chunkXVMTOYRUjs.loadTsupConfig.call(void 0, 
    process.cwd(),
  ...
  await Promise.all(
    [...Array.isArray(configData) ? configData : [configData]].map(
      async (item) => {
        const logger3 = _chunkXVMTOYRUjs.createLogger.call(void 0, _optionalChain([item, 'optionalAccess', _33 => _33.name]));
        const options = await normalizeOptions(logger3, item, _options);
        logger3.info("CLI", `tsup v${_chunkDDBRY6AFjs.version}`);
        if (config.path) {
        ...
        const mainTasks = async () => {
          if (!_optionalChain([options, 'access', _35 => _35.dts, 'optionalAccess', _36 => _36.only])) {
            ...
            let depsHash = await _chunkXVMTOYRUjs.getAllDepsHash.call(void 0, process.cwd());
              ...
              watcher.on("all", async (type, file) => {
                ...
                if (options.watch === true) {
                  if (file === "package.json" && !buildDependencies.has(file)) {
                    const currentHash = await _chunkXVMTOYRUjs.getAllDepsHash.call(void 0, process.cwd());
