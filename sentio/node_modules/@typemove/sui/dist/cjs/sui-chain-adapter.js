...
class SuiChainAdapter extends move_1.ChainAdapter {
    ...
    toInternalModules(modules) {
        return Object.values(modules).map(to_internal_js_1.toInternalModule);
    }
        ...
        for (const module of modules) {
            ...
            for (const struct of module.structs) {
                ...
                if (abilities.has('Drop') && abilities.has('Copy')) {
                    eventMap.set(qname + move_1.SPLITTER + struct.name, struct);
                }
            }
        }
        return eventMap;
...
exports.SuiChainAdapter = SuiChainAdapter;
function getRpcClient(endpoint) {
    return new client_1.SuiClient({ url: endpoint });
}
