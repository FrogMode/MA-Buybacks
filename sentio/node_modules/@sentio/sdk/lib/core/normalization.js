...
import { CoinID, RichValue_NullValue, TokenAmount } from '@sentio/protos';
...
function normalizeObject(obj, length, path) {
    let ret;
    ...
    if (Array.isArray(obj)) {
        console.warn(`Array type inside log/event payload is not currently supported and will be ignored. Path: ${path ?? ''}`);
        return null;
        // ret = []
...
function normalizeToRichValue(value) {
    if (value == null) {
...
export function normalizeToRichStruct(...objs) {
    const ret = {
        fields: {}
    };
