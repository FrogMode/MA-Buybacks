...
export class EthFacet {
    ...
    testTrace(trace, network = EthChainId.ETHEREUM) {
        return this.testTraces([trace], network);
    }
    testTraces(traces, network = EthChainId.ETHEREUM) {
        const bindings = [];
    ...
    buildTraceBinding(trace, network = EthChainId.ETHEREUM) {
    ...
    testLog(log, network = EthChainId.ETHEREUM) {
        return this.testLogs([log], network);
    }
    testLogs(logs, network = EthChainId.ETHEREUM) {
        const bindings = [];
    ...
    buildLogBinding(log, network = EthChainId.ETHEREUM) {
        for (const contract of this.server.contractConfigs) {
            if (contract.contract?.chainId !== network) {
                continue;
            }
    ...
    testAccountLog(address, log, network = EthChainId.ETHEREUM) {
        return this.testAccountLogs(address, [log], network);
    }
    testAccountLogs(address, logs, network = EthChainId.ETHEREUM) {
        const bindings = [];
        for (const log of logs) {
    ...
    buildAccountLogBinding(address, log, network = EthChainId.ETHEREUM) {
    ...
    testBlock(block, network = EthChainId.ETHEREUM) {
        return this.testBlocks([block], network);
    }
    testBlocks(blocks, network = EthChainId.ETHEREUM) {
        const bindings = [];
    ...
    buildBlockBinding(block, network = EthChainId.ETHEREUM) {
        const binding = {
            data: {
                ethBlock: { block }
            },
    ...
    testTransaction(transaction, network = EthChainId.ETHEREUM) {
        return this.testTransactions([transaction], network);
    }
    testTransactions(transactions, network = EthChainId.ETHEREUM) {
        const bindings = [];
    ...
    buildTransactionBinding(transaction, network = EthChainId.ETHEREUM) {
        const binding = {
            data: {
...
//# sourceMappingURL=eth-facet.js.map