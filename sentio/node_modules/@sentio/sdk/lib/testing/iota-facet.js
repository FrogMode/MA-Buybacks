...
export class IotaFacet {
    ...
    testEntryFunctionCall(transaction, network = IotaNetwork.MAIN_NET) {
        return this.testEntryFunctionCalls([transaction], network);
    }
    testEntryFunctionCalls(transactions, network = IotaNetwork.MAIN_NET) {
        const bindings = [];
    ...
    buildEntryFunctionCallBinding(transaction, network = IotaNetwork.MAIN_NET) {
    ...
    testEvent(transaction, network = IotaNetwork.MAIN_NET) {
    ...
    testGlobalTransaction(transaction, network = IotaNetwork.MAIN_NET) {
        const handlerIds = [];
        for (const config of this.server.contractConfigs) {
    ...
    buildEventBinding(transaction, network = IotaNetwork.MAIN_NET) {
            ...
            for (const eventConfig of config.moveEventConfigs) {
                for (const eventFilter of eventConfig.filters) {
                    for (const event of transaction.events || []) {
                        if (accountTypeString(config.contract.address) + '::' + eventFilter.type ===
...
//# sourceMappingURL=iota-facet.js.map