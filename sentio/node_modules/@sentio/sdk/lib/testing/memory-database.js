...
export class MemoryDatabase {
    ...
    fillDerivedFromFields(entity, id, result) {
        ...
        if (entityClass && entityClass instanceof GraphQLObjectType) {
            for (const field of Object.values(entityClass.getFields())) {
                ...
                if (derivedFrom) {
                    const elemType = getElemType(field.type);
                            ...
                            if (elemDb) {
                                for (const elemID of Object.keys(elemDb)) {
                                    ...
                                    if (deriveField?.listValue) {
                                        hasRelation = deriveField.listValue.values.some((v) => v.stringValue === id);
                                    }
                                    if (hasRelation) {
