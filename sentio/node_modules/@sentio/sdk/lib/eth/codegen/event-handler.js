import { getFullSignatureAsSymbolForEvent } from 'typechain';
...
export function generateEventHandler(event, contractName, includeArgTypes) {
...
export function generateEventHandlers(events, contractName) {
    if (events.length === 1) {
...
function generateEventFilter(event, includeArgTypes) {
    const eventName = includeArgTypes ? getFullSignatureAsSymbolForEvent(event) : event.name;
    const eventNamePrefix = includeArgTypes ? eventName + '_' : eventName;
    const filterName = getFullSignatureForEventPatched(event);
    return `
...
export function generateEventFilters(events) {
    if (events.length === 1) {
