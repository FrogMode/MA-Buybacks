...
import { getFullSignatureAsSymbolForFunction, getFullSignatureForFunction, getFullSignatureWithOutputForFn } from './types.js';
...
export function generateFunctionCallEncoders(functions) {
    const includeArgTypes = functions.length !== 1;
    return functions.flatMap((fn) => generateFunctionCallEncoder(fn, includeArgTypes));
}
export function generateFunctionCallEncoder(fn, includeArgTypes) {
    const declName = includeArgTypes ? getFullSignatureAsSymbolForFunction(fn) : fn.name;
    return [
        `
  ${declName}(${generateInputTypes(fn.inputs, {
    ...
    return encodeCallData(callContext, "${fn.name}", "function ${getSignatureForFn(fn)}", [${fn.inputs.length > 0 ? fn.inputs.map((input, index) => input.name || `arg${index}`).join(',') + ',' : ''}])
  }
  `
    ];
}
export function generateBoundFunctionCallEncoders(functions) {
    const includeArgTypes = functions.length !== 1;
    return functions.flatMap((fn) => generateBoundFunctionCallEncoder(fn, includeArgTypes));
}
export function generateBoundFunctionCallEncoder(fn, includeArgTypes) {
    const declName = includeArgTypes ? getFullSignatureAsSymbolForFunction(fn) : fn.name;
  ...
  ${declName ?? fn.name}(${generateInputTypes(fn.inputs, {
    ...
    return this.view.encodeCall.${declName}(${fn.inputs.length > 0 ? fn.inputs.map((input, index) => input.name || `arg${index}`).join(',') + ',' : ''}{chainId: this.context.chainId.toString(), address: this.context.address, blockTag: this.context.getBlockTag(overrides)})
  }
  `
    ];
}
