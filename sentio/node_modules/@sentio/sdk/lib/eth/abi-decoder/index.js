import { LogDescription, Result } from 'ethers';
import { GLOBAL_CONFIG, processMetrics } from '@sentio/runtime';
import { Piscina } from 'piscina';
import { decodeTraceInline, parseLogInline } from './decode-worker.js';
...
export async function parseLog(processor, log) {
    if (GLOBAL_CONFIG.execution.ethAbiDecoderConfig?.enabled) {
        ...
        try {
            const result = (await workerPool.run({ log }, { name: 'parseLog' }));
            if (result) {
...
export async function decodeTrace(processor, inputs, traceData) {
    if (GLOBAL_CONFIG.execution.ethAbiDecoderConfig?.enabled) {
        ...
        const start = Date.now();
        try {
            const result = (await workerPool.run({ inputs, traceData }, { name: 'decodeTrace' }));
            if (result) {
