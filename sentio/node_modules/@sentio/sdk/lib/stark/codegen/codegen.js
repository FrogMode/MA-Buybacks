...
async function codegenInternal(abisDir, outDir) {
    const allFiles = fs.readdirSync(abisDir, { recursive: true });
    const abis = {};
    let fileCount = 0;
    function guessNameFromAbi(abi, address) {
    ...
    for (const { name, address, chain, abi: jsonAbi } of Object.values(abis)) {
        const content = [];
        content.push(`import { StarknetProcessorConfig, StarknetEvent, AbstractStarknetProcessor, StarknetContext } from '@sentio/sdk/starknet'`);
