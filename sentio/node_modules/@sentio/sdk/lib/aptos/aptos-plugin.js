...
export class AptosPlugin extends Plugin {
    ...
    async start(request) {
        await initTokenList();
        const allowedChainIds = new Set(Object.values(AptosChainId));
        for (const instance of request.templateInstances) {
            if (!allowedChainIds.has(instance.contract?.chainId || '')) {
                continue;
            }
            const template = AptosResourceProcessorTemplateState.INSTANCE.getValues()[instance.templateId];
    ...
    async processAptosEvent(binding) {
        ...
        for (const handlerId of binding.handlerIds) {
            const promise = this.handlerRegister
                .getHandlerById(binding.chainId, handlerId)(event)
                .catch((e) => {
    ...
    async processAptosResource(binding) {
        ...
        for (const handlerId of binding.handlerIds) {
            const promise = this.handlerRegister
                .getHandlerById(binding.chainId, handlerId)(resource)
                .catch((e) => {
    ...
    async processAptosFunctionCall(binding) {
        ...
        for (const handlerId of binding.handlerIds) {
            ...
            const promise = this.handlerRegister
                .getHandlerById(binding.chainId, handlerId)(call)
                .catch((e) => {
